...
//
// Grammars for Project Milestone 3.
//
// Skeleton 
// * SubScript Grammar
// * MinARM32 assembler grammar
// * Helpers for MinARM32 generation
// * Compiler (To Be Done)
//
// Based on
//   http://cs.nyu.edu/courses/fall15/CSCI-GA.2130-001/project2/Pr2Rose.hx
//   http://cs.nyu.edu/courses/fall15/CSCI-GA.2130-001/MinARM32.pdf
//
// This file is Copyright © 2015 Kristoffer Rose ⟨krisrose@crsx.org⟩
// and licensed under the CC-BY-4.0 license.

module edu.nyu.cs.cc.Pr3Yu {
  ...
  //
  // [2] references are to the Pr2Rose.pdf solution.
  ...
  // [2]1.8. SYNTAX-DIRECTED DEFINITION
  //
  ...
  // | E1.te = S2.te = S.te
  // | S2.rt = S.rt
  Stert(⟦ while ( ⟨Expression#1⟩ ) ⟨Statement#2⟩ ⟧↑#syn)
  ...
  sort Instruction
    | ⟦ DEF ⟨IDENTIFIER⟩ = ⟨INTEGER⟩ ¶⟧	// define identifier
    | ⟦ ⟨Label⟩: ¶⟧ 		 		// define address label
    | ⟦ ⟨IDENTIFIER⟩: ¶⟧
    | ⟦ DCI ⟨INTEGER⟩ ¶⟧			// allocate integer
    | ⟦ DCB ⟨INTEGER⟩ ¶⟧			// allocate byte
    | ⟦ DCS ⟨STRING⟩ ¶⟧			// allocate string
    | ⟦ ⟨Op⟩ ¶⟧	  			// machine instruction
    ;

  sort Label | symbol ⟦⟨IDENTIFIER⟩⟧ ;
  ...
  sort Op
    ...
    | ⟦ BLX ⟨Reg⟩ ⟧				// branch and link through register
  ...
  sort Regs | ⟦⟨Reg⟩⟧ | ⟦⟨Reg⟩, ⟨Regs⟩⟧ | ⟦⟨Reg⟩-⟨Reg⟩⟧ | ⟦⟨Reg⟩-⟨Reg⟩, ⟨Regs⟩⟧ ;

  sort Constant | ⟦#⟨INTEGER⟩⟧ | ⟦&⟨IDENTIFIER⟩⟧ ;

  
  ////////////////////////////////////////////////////////////////////////
  // MinARM32 ASSEMBLER HELPERS

  // Special syntax to concatenate Instructions.
  ...
  sort Rs | RsNil | RsCons(Reg, Rs) | scheme AppendRs(Rs, Rs) ;
  AppendRs(RsNil, #Rs) → #Rs ;
  AppendRs(RsCons(#Rn, #Rs1), #Rs2) → RsCons(#Rn, AppendRs(#Rs1, #Rs2)) ;

  // Helper conversion from Regs syntax to list of registers.
  | scheme XRegs(Regs) ;
  XRegs(⟦⟨Reg#r⟩⟧) → RsCons(#r, RsNil) ;
  XRegs(⟦⟨Reg#r1⟩-⟨Reg#r2⟩⟧) → XRegs1(#r1, #r2) ;
  XRegs(⟦⟨Reg#r⟩, ⟨Regs#Rs⟩⟧) → RsCons(#r, XRegs(#Rs)) ;
  XRegs(⟦⟨Reg#r1⟩-⟨Reg#r2⟩, ⟨Regs#Rs⟩⟧) → AppendRs(XRegs1(#r1, #r2), XRegs(#Rs)) ;

  | scheme XRegs1(Reg, Reg) ;
  XRegs1(#r, #r) → RsCons(#r, RsNil) ;
  default XRegs1(#r1, #r2) → XRegs2(#r1, #r2) ;

  | scheme XRegs2(Reg, Reg) ;
  XRegs2(⟦R0⟧, #r2) → RsCons(⟦R0⟧, XRegs1(⟦R1⟧, #r2)) ;
  XRegs2(⟦R1⟧, #r2) → RsCons(⟦R1⟧, XRegs1(⟦R2⟧, #r2)) ;
  XRegs2(⟦R2⟧, #r2) → RsCons(⟦R2⟧, XRegs1(⟦R3⟧, #r2)) ;
  XRegs2(⟦R3⟧, #r2) → RsCons(⟦R3⟧, XRegs1(⟦R4⟧, #r2)) ;
  XRegs2(⟦R4⟧, #r2) → RsCons(⟦R4⟧, XRegs1(⟦R5⟧, #r2)) ;
  XRegs2(⟦R5⟧, #r2) → RsCons(⟦R5⟧, XRegs1(⟦R6⟧, #r2)) ;
  XRegs2(⟦R6⟧, #r2) → RsCons(⟦R6⟧, XRegs1(⟦R7⟧, #r2)) ;
  XRegs2(⟦R7⟧, #r2) → RsCons(⟦R7⟧, XRegs1(⟦R8⟧, #r2)) ;
  XRegs2(⟦R8⟧, #r2) → RsCons(⟦R8⟧, XRegs1(⟦R9⟧, #r2)) ;
  XRegs2(⟦R9⟧, #r2) → RsCons(⟦R9⟧, XRegs1(⟦R10⟧, #r2)) ;
  XRegs2(⟦R10⟧, #r2) → RsCons(⟦R10⟧, XRegs1(⟦R11⟧, #r2)) ;
  XRegs2(⟦R11⟧, #r2) → RsCons(⟦R11⟧, XRegs1(⟦R12⟧, #r2)) ;
  XRegs2(⟦R12⟧, #r2) → RsCons(⟦R12⟧, RsNil) ;
  ...
  // Helper to generate instruction that adds a computed constant to a register.
  // Note the special pattern for sorts.
  ...
  // Helpers to insert computed constants.
  sort Constant | scheme Immediate(Computed) ;
  Immediate(#x) → ⟦#⟨INTEGER#x⟩⟧ ;

  // Helper to convert INTEGER to Computed number.
  sort Computed | scheme ToNum(INTEGER);
  ToNum(#) → ⟦ $# ⟧ ;

  ////////////////////////////////////////////////////////////////////////
  // COMPILER (TO BE DONE)

  sort PassOne | ⟦ ⟨Instructions⟩ - ⟨NameTypeListTail⟩ ⟧;

  sort PassOne | scheme FirstPass(Program);
  FirstPass(⟦ ⟨Units#Us⟩ ⟧) → FirstPassUs(#Us);
  FirstPass(⟦ ⟨Units#Us⟩ ⟧) → AppendPO(FirstPassUs(#Us), ExtendGlobalDefs);

  // extends the global functions into 
  sort PassOne | scheme ExtendGlobalDefs;
  ExtendGlobalDefs → ⟦ -
    , document : {body : {innerHTML : string}}
    , length : (string) => number
    , charAt : (string, number) => number
  ...
  sort PassOne | scheme FirstPassUs(Units);
  FirstPassUs(⟦ ⟨Unit#1⟩ ⟨Units#2⟩ ⟧) → AppendPO(FirstPassU(#1), FirstPassUs(#2));
  FirstPassUs(⟦ ⟨Unit#⟩ ⟧) → FirstPassU(#);

  // [3] 1.1.4 collecting S.uds into the global type environment
  sort PassOne | scheme ExtendNameTypes(NameTypeListTail, PassOne);
  ExtendNameTypes(#1, ⟦ ⟨Instructions#21⟩ - ⟨NameTypeListTail#22⟩ ⟧)
    → ⟦ ⟨Instructions#21⟩ - ⟨NameTypeListTail AppendNameTypes(#1, #22)⟩ ⟧;

  sort PassOne | scheme AppendPO(PassOne, PassOne);
  AppendPO(⟦ ⟨Instructions#1⟩ - ⟨NameTypeListTail#2⟩⟧, ⟦ ⟨Instructions#3⟩ - ⟨NameTypeListTail#4⟩⟧)
    → ⟦ {⟨Instructions#1⟩} ⟨Instructions#3⟩ - ⟨NameTypeListTail AppendNameTypes(#2, #4)⟩⟧;

  sort PassOne | scheme FirstPassU(Unit);
  FirstPassU(⟦ ⟨Declaration#⟩ ⟧) → FirstPassDs(#);
  FirstPassU(⟦ ⟨Statement# ↑ds(#ds)⟩ ⟧) → ⟦ - ⟨NameTypeListTail#ds⟩ ⟧;

  sort PassOne | scheme FirstPassDs(Declaration);
  FirstPassDs(⟦ interface ⟨IDENTIFIER#1⟩ { ⟨Members#2⟩ } ⟧)
    → MemberTypeHelper(#1, #2, ⟦0⟧);
  FirstPassDs(⟦ function ⟨IDENTIFIER#1⟩ ⟨CallSignature#2 ↑t(#2t)⟩ { ⟨Statements#3⟩ } ⟧)
    → ⟦  - , ⟨IDENTIFIER#1⟩ : ⟨Type#2t⟩⟧;

  sort PassOne | scheme  MemberTypeHelper(IDENTIFIER, Members, Computed);
  MemberTypeHelper(#id, ⟦ ⟨IDENTIFIER#21⟩ ⟨CallSignature#22⟩ { ⟨Statements#23⟩ } ⟨Members#24⟩ ⟧, #num)
    → AppendPO(MemberOffsetGen(MemberNameConCat(#id, #21), #22, #num), MemberTypeHelper(#id, #24, ⟦#num+4⟧));
  MemberTypeHelper(#id, ⟦ ⟨IDENTIFIER#21⟩ : ⟨Type#22⟩ ; ⟨Members#23⟩ ⟧, #num)
    → AppendPO(MemberOffsetGen(MemberNameConCat(#id, #21), #22, #num),MemberTypeHelper(#id, #23, ⟦#num+4⟧));
  MemberTypeHelper(#id, ⟦⟧, #num) → InterfaceSizeGen(#id, #num);

  sort Computed | scheme MemberNameConCat(IDENTIFIER, IDENTIFIER);
  MemberNameConCat(#1, #2) → ⟦#1 @ "_" @ #2 @ "_offset"⟧;

  sort PassOne | scheme MemberOffsetGen(Computed, Type, Computed);
  MemberOffsetGen(#id, #t, #num) → ⟦ DEF ⟨IDENTIFIER#id⟩ = ⟨INTEGER#num⟩ - , ⟨IDENTIFIER#id⟩ : ⟨Type#t⟩ ⟧;

  sort PassOne | scheme InterfaceSizeGen(IDENTIFIER, Computed);
  InterfaceSizeGen(#id, #num)
    → ⟦ DEF ⟨IDENTIFIER InterfaceNameConCat(#id)⟩ = ⟨INTEGER#num⟩ -  ⟧;
  
  sort Computed | scheme InterfaceNameConCat(IDENTIFIER);
  InterfaceNameConCat(#id) → ⟦ #id @ "__size"⟧;

  // Main scheme. The Second Pass
  sort Instructions |  scheme Compile(Program);
  Compile(#) → ⟦
    Main:
    { ⟨Instructions ExtractInsts(FirstPass(P2(#)))⟩}
    ⟨Instructions SecondPass(FirstPass(P2(#)), P2(#))⟩
  ⟧;

  // helper syntactic sort to save the offset of var NT
  sort Emap | ⟦ ⟨NameType⟩ - ⟨INTEGER⟩ - ⟨RegList⟩⟧ | ⟦ ⟨NameType⟩ - ⟨RegList⟩ ⟧;

  sort Emaps | ⟦ ⟨Emap⟩ ⟨Emaps⟩ ⟧ | ⟦⟧;

  // helper sort to save unused registers
  sort RegList | ⟦ ⟨Reg⟩, ⟨RegList⟩⟧ | ⟦⟧;

  sort RegList | scheme InitRegList;
  InitRegList → ⟦R0, R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11, R12, ⟧;

  // helper sort to initialize the parameters passed to the parameters
  sort RegList | scheme ArgRegList;
  ArgRegList → ⟦R4, R5, R6, R7, R8, R9, R10, R11, R12, ⟧;

  sort Instructions | scheme ExtractInsts(PassOne);
  ExtractInsts(⟦ ⟨Instructions#1⟩ - ⟨NameTypeListTail#2⟩ ⟧) → #1;

  sort NameTypeListTail | scheme ExtractNTLT(PassOne);
  ExtractNTLT(⟦ ⟨Instructions#1⟩ - ⟨NameTypeListTail#2⟩ ⟧) → #2;

  sort Instructions | scheme SecondPass(PassOne, Program);
  SecondPass(#1, ⟦ ⟨Unit#2⟩ ⟧) → ⟦
    MOV R12, SP
    ⟨Instructions CompileUnit(#1, #2, ⟦L⟧, ⟦-4⟧)⟩
  ⟧;
  SecondPass(#1, ⟦ ⟨Unit#21⟩ ⟨Units#22⟩ ⟧) → ⟦
    MOV R12, SP
    { ⟨Instructions CompileUnit(#1, #21, ⟦L⟧, ⟦-4⟧)⟩}
    B L
    ⟨Instructions CompileUnits(#1, #22, ComputeUnit(#21, ⟦-4⟧))⟩
  ⟧;

  sort Instructions | scheme CompileUnits(PassOne, Units, Computed);
  CompileUnits(#1,  ⟦ ⟨Unit#2⟩ ⟧, #offset) → ⟦
    { ⟨Instructions CompileUnit(#1, #2, ⟦Ls⟧, #offset)⟩ }
    B Ls
  ⟧;
  CompileUnits(#1, ⟦ ⟨Unit#21⟩ ⟨Units#22⟩⟧, #offset) → ⟦
    { ⟨Instructions CompileUnit(#1, #21, ⟦L⟧, #offset)⟩}
    B L
    ⟨Instructions CompileUnits(#1, #22, ComputeUnit(#21, #offset))⟩
  ⟧;

  sort Instructions | scheme CompileUnit(PassOne, Unit, Label, Computed);
  CompileUnit(#1, ⟦ ⟨Declaration#2⟩ ⟧, #3, #4) → CompileD(#1, #2, #3, #4, ⟦⟧, InitRegList);
  CompileUnit(#1, ⟦ ⟨Statement#2⟩ ⟧, #3, #4) → CompileS(#1, #2, #3, #4, ⟦⟧, InitRegList);

  // [3]1.2.4
  sort Instructions | scheme CompileD(PassOne, Declaration, Label, Computed, Emaps, RegList);
  CompileD(#1, ⟦ interface ⟨IDENTIFIER#21⟩ { ⟨Members#22⟩ } ⟧, #3, #4, #5, #6)
    → CompileMs(#1, #22, #3, #4, #5, #6, #21);
  CompileD(#1, ⟦ function ⟨IDENTIFIER#21⟩ ⟨CallSignature#22⟩ { ⟨Statements#23⟩ } ⟧, #3, #4, #5, #6)
    → ⟦ B ⟨Label#3⟩
	⟨IDENTIFIER#21⟩: STMFD SP!, {R4-R11, LR}
	                 MOV R12, SP
	{ ⟨Instructions CompileSs(#1, #23, ⟦L2⟧, #4, ExtendEmaps(#22, #5, InitRegList), #6)⟩}
	L2:  MOV SP, R12
	    LDMFD SP!, {R4-R11, PC}
  ⟧;

  // helper scheme to extend Cs into Emaps
  sort Emaps | scheme ExtendEmaps(CallSignature, Emaps, RegList);
  ExtendEmaps(⟦ ( ) : ⟨Type#22⟩ ⟧, #5, #6) → #5;
  default ExtendEmaps(⟦ ( ⟨IDENTIFIER#21⟩ : ⟨Type#22⟩ ⟨NameTypeListTail#23⟩ ) : ⟨Type#24⟩ ⟧, #5, #6)
    → ExtendEmaps(⟦ ( ⟨NameTypeList ToNTL(#23)⟩ ) : ⟨Type#24⟩ ⟧, ⟦ ⟨IDENTIFIER#21⟩ : ⟨Type#22⟩ - ⟨Reg GetHeadInRegList(#6)⟩, ⟨Emaps#5⟩⟧, RemoveHeadInRegList(#6));
  
  // helper scheme to convert NTLT to NTL
  sort NameTypeList | scheme ToNTL(NameTypeListTail);
  ToNTL(⟦⟧) → ⟦⟧;
  ToNTL(⟦ , ⟨NameType#21⟩ ⟨NameTypeListTail#22⟩ ⟧) → ⟦ ⟨NameType#21⟩ ⟨NameTypeListTail#22⟩ ⟧;

  // [3]1.2.7
  sort Instructions | scheme CompileMs(PassOne, Members, Label, Computed, Emaps, RegList, IDENTIFIER);
  CompileMs(#1, ⟦⟧, #3, #4, #5, #6, #7) → ⟦⟧;
  CompileMs(#1, ⟦ ⟨Member#21⟩ ⟨Members#22⟩ ⟧, #3, #4, #5, #6, #7)
    → ⟦ {⟨Instructions CompileM(#1, #21, #3, #4, #5, #6, #7)⟩}
    ⟨Instructions CompileMs(#1, #22, #3, #4, #5, #6, #7)⟩
  ⟧;

  // [3]1.2.6
  sort Instructions | scheme CompileM(PassOne, Member, Label, Computed, Emaps, RegList, IDENTIFIER);
  CompileM(#1, ⟦ ⟨IDENTIFIER#21⟩ : ⟨Type#22⟩ ; ⟧, #3, #4, #5, #6, #7) → ⟦⟧;
  CompileM(#1, ⟦ ⟨IDENTIFIER#21⟩ ⟨CallSignature#22⟩ { ⟨Statements#23⟩ } ⟧, #3, #4, #5, #6, #7)
    → ⟦ ⟨IDENTIFIER ConCatEntry(#7, #21)⟩:
      STMFD SP!, {R4-R11, LR}
      MOV R12, SP
      {⟨Instructions CompileSs(#1, #23, ⟦Lm⟧, #4, ExtendEmaps(#22, #5, InitRegList), #6)⟩}
      Lm: MOV SP, R12
      LDMFD SP!, {R4-R11, PC}
  ⟧;

  sort Computed | scheme ConCatEntry(IDENTIFIER, IDENTIFIER);
  ConCatEntry(#1, #2) → ⟦#1 @ "_" @ #2 @ "_entry"⟧;

  sort Instructions | scheme CompileS(PassOne, Statement, Label, Computed, Emaps, RegList);
  CompileS(#1, ⟦ ⟨Expression#2⟩ ; ⟧, #3, #4, #5, #6)
    → CompileE(#1, #2, #3, #4, #5, #6);
  CompileS(#1, ⟦ { ⟨Statements#2⟩ } ⟧, #3, #4, #5, #6) → CompileSs(#1, #2, #3, #4, #5, #6);
  CompileS(#1, ⟦ var ⟨NameType#2⟩ ; ⟧, #3, #4, #5, #6) → ⟦ SUB SP, R12, #⟨INTEGER NegToPos(⟦#4-4⟧)⟩⟧;
  CompileS(#1, ⟦ ; ⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileS(#1, ⟦ if ( ⟨Expression#21⟩ )  ⟨Statement#22⟩  ⟧, #3, #4, #5, #6)
    → ⟦ {⟨Instructions CompileCE(#1, #21, #3, #4, #5, #6, ⟦L1⟧, ⟦L2⟧)⟩}
      L1: {⟨Instructions CompileS(#1, #22, #3, #4, #5, #6)⟩}
      L2: 
  ⟧;
  CompileS(#1, ⟦ if ( ⟨Expression#21⟩ ) ⟨Statement#22⟩ else ⟨Statement#23⟩ ⟧, #3, #4, #5, #6)
    → ⟦ {{{⟨Instructions CompileCE(#1, #21, #3, #4, #5, #6, ⟦L1⟧, ⟦L2⟧)⟩}
       L1:
      ⟨Instructions CompileS(#1, #22, #3, #4, #5, #6)⟩}
       L2:
      ⟨Instructions CompileS(#1, #23, #3, #4, #5, #6)⟩}
       L3: ADD SP, R12, #⟨INTEGER AddFour(#4)⟩
      ⟧;
  CompileS(#1, ⟦ while ( ⟨Expression#21⟩ ) ⟨Statement#22⟩ ⟧, #3, #4, #5, #6)
    → ⟦ B L2
        L1: 
	{⟨Instructions CompileS(#1, #22, #3, #4, #5, #6)⟩}
        L2: {⟨Instructions CompileCE(#1, #21, #3, #4, #5, #6, ⟦L2⟧, ⟦L3⟧)⟩}
        L3: ADD SP, R12, #⟨INTEGER AddFour(#4)⟩
	⟧;
  CompileS(#1,  ⟦ return ⟨Expression#2⟩ ; ⟧, #3, #4, #5, #6)
    → ⟦ {⟨Instructions CompileE(#1, #2, #3, #4, #5, #6)⟩}
      MOV R0, ⟨Reg GetHeadInRegList(#6)⟩
      B ⟨Label #3⟩
   ⟧;
  CompileS(#1,  ⟦ return ; ⟧, #3, #4, #5, #6) → ⟦ B ⟨Label #3⟩ ⟧;
  // CompileS(#1, #2, #3, #4, #5, #6) → error ⟦selected⟧;
  
  sort Instructions | scheme CompileSs(PassOne, Statements, Label, Computed, Emaps, RegList);
  CompileSs(#1, ⟦⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileSs(#1, ⟦ ⟨Statement#21⟩ ⟨Statements#22⟩ ⟧, #3, #4, #5, #6) → ⟦
    { ⟨Instructions CompileS(#1, #21, #3, #4, ExtendSHelper(#21, #4, #5), #6)⟩}
    ADD SP, R12, #⟨INTEGER NegToPos(⟦#4+4⟧)⟩
    ⟨Instructions CompileSs(#1, #22, #3, ComputeS(#21, #4), #5, #6)⟩
  ⟧;

  sort Instructions | scheme CompileE(PassOne, Expression, Label, Computed, Emaps, RegList);
  CompileE(#1, ⟦ ⟨INTEGER#2⟩ ⟧, #3, #4, #5, #6) →  CompileInt(#2, #6);
  CompileE(#1, ⟦ ⟨STRING#2⟩ ⟧, #3, #4, #5, #6)
    → ⟦ ADD ⟨Reg GetHeadInRegList(#6)⟩, PC, #0
        B N
	DCS ⟨STRING#2⟩
	N:
      ⟧;
  CompileE(#1, ⟦ ⟨IDENTIFIER#2⟩ ⟧, #3, #4, #5, #6)
    → ⟦LDR ⟨Reg GetHeadInRegList(#6)⟩, [R12, #4]⟧; //--- CompileId(#2, #5, #6);
  CompileE(#1, ⟦ + ⟨Expression#2⟩ ⟧, #3, #4, #5, #6) → CompileE(#1, #2, #3, #4, #5, #6);
  CompileE(#1, ⟦ - ⟨Expression#2⟩ ⟧, #3, #4, #5, #6)
    → ⟦ {⟨Instructions CompileE(#1, #2, #3, #4, #5, #6)⟩}
    RSB ⟨Reg GetHeadInRegList(#6)⟩, ⟨Reg GetHeadInRegList(#6)⟩, #0 ⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩ ( ⟨ExpressionList#22⟩ ) ⟧, #3, #4, #5, #6) → ⟦
    {{⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
    ⟨Instructions CompileEinEL(#1, #22, #3, #4, #5, ArgRegList)⟩}
    STMFD SP!, {R0-R3, R12}
    {⟨Instructions MovEL(#22, ⟦R0⟧, ArgRegList)⟩}
    BLX ⟨Reg GetHeadInRegList(ArgRegList)⟩
    MOV ⟨Reg GetHeadInRegList(#6)⟩, R0
    LDMFD SP!, {R0-R3, R12}
  ⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩ * ⟨Expression#22⟩ ⟧, #3, #4, #5, #6)
    → ⟦ {{⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
      ⟨Instructions CompileE(#1, #22, #3, #4, #5, RemoveHeadInRegList(#6))⟩}
      MUL ⟨Reg GetHeadInRegList(#6)⟩, ⟨Reg GetHeadInRegList(RemoveHeadInRegList(#6))⟩, ⟨Reg GetHeadInRegList(#6)⟩
  ⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩ - ⟨Expression#22⟩ ⟧, #3, #4, #5, #6)
    → ⟦ {{⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
      ⟨Instructions CompileE(#1, #22, #3, #4, #5, RemoveHeadInRegList(#6))⟩}
      SUB ⟨Reg GetHeadInRegList(#6)⟩, ⟨Reg GetHeadInRegList(RemoveHeadInRegList(#6))⟩, ⟨Reg GetHeadInRegList(#6)⟩
  ⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩ & ⟨Expression#22⟩ ⟧, #3, #4, #5, #6)
    → ⟦ {{⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
      ⟨Instructions CompileE(#1, #22, #3, #4, #5, RemoveHeadInRegList(#6))⟩}
      AND ⟨Reg GetHeadInRegList(#6)⟩, ⟨Reg GetHeadInRegList(RemoveHeadInRegList(#6))⟩, ⟨Reg GetHeadInRegList(#6)⟩
  ⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩ | ⟨Expression#22⟩ ⟧, #3, #4, #5, #6)
    → ⟦ {{⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
      ⟨Instructions CompileE(#1, #22, #3, #4, #5, RemoveHeadInRegList(#6))⟩}
      ORR ⟨Reg GetHeadInRegList(#6)⟩, ⟨Reg GetHeadInRegList(RemoveHeadInRegList(#6))⟩, ⟨Reg GetHeadInRegList(#6)⟩
  ⟧;
  // no code generated for / and % because they are calculated using libraries.
  CompileE(#1, ⟦ ⟨Expression#21⟩ / ⟨Expression#22⟩ ⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩ % ⟨Expression#22⟩ ⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩ ^ ⟨Expression#22⟩ ⟧, #3, #4, #5, #6)
    → ⟦ {{⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
      ⟨Instructions CompileE(#1, #22, #3, #4, #5, RemoveHeadInRegList(#6))⟩}
      EOR ⟨Reg GetHeadInRegList(#6)⟩, ⟨Reg GetHeadInRegList(RemoveHeadInRegList(#6))⟩, ⟨Reg GetHeadInRegList(#6)⟩
  ⟧;
  CompileE(#1, ⟦ ~ ⟨Expression#2⟩ ⟧, #3, #4, #5, #6)
    → ⟦ {⟨Instructions CompileE(#1, #2, #3, #4, #5, #6)⟩}
      RSB ⟨Reg GetHeadInRegList(#6)⟩, ⟨Reg GetHeadInRegList(RemoveHeadInRegList(#6))⟩, ⟨Reg GetHeadInRegList(#6)⟩
  ⟧;
  CompileE(#1, ⟦ ⟨IDENTIFIER#21⟩ = ⟨Expression#22⟩ ⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩.⟨IDENTIFIER#21⟩ = ⟨Expression#23⟩ ⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileE(#1, ⟦ ⟨Expression#21↑t(#t)⟩ . ⟨IDENTIFIER#22⟩ ⟧, #3, #4, #5, #6)
    → ⟦ LDR ⟨Reg GetHeadInRegList(#6)⟩, [⟨Reg GetHeadInRegList(#6)⟩, &⟨IDENTIFIER MemberNameConCat2(#t, #22)⟩] ⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩ + ⟨Expression#21⟩ ⟧, #3, #4, #5, #6) → ⟦⟧;  
  CompileE(#1, ⟦ ⟨Expression#21⟩ ? ⟨Expression#22⟩ : ⟨Expression#23⟩ ⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩ += ⟨Expression#22⟩ ⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileE(#1, ⟦ ⟨Expression#21⟩ = { ⟨KeyValueList#22⟩ } ⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileE(#1, #2, #3, #4, #5, #6)
    → ⟦ {⟨Instructions CompileCE(#1, #2, #3, #4, #5, #6, ⟦L1⟧, ⟦L2⟧)⟩}
      L1: MOV ⟨Reg GetHeadInRegList(#6)⟩, #1
      B X
      L2: MOV ⟨Reg GetHeadInRegList(#6)⟩, #0
      X:
  ⟧;
  
   // helper scheme to generate code for Conditional Expression
  sort Instructions | scheme CompileCE(PassOne, Expression, Label, Computed, Emaps, RegList, Label, Label);
  CompileCE(#1, ⟦ ⟨Expression#21⟩ < ⟨Expression#22⟩ ⟧, #3, #4, #5, #6, #true, #false)
    → ⟦ {{⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
      ⟨Instructions CompileE(#1, #22, #3, #4, #5, RemoveHeadInRegList(#6))⟩}
      CMP ⟨Reg GetHeadInRegList(#6)⟩, ⟨Reg GetHeadInRegList(RemoveHeadInRegList(#6))⟩
      BLT ⟨Label#true⟩
      B ⟨Label#false⟩
  ⟧;
  CompileCE(#1, ⟦ ! ⟨Expression#2⟩ ⟧, #3, #4, #5, #6, #true, #false)
    → CompileCE(#1, #2, #3, #4, #5, #6, #false, #true);
  CompileCE(#1, ⟦ ⟨Expression#21⟩ > ⟨Expression#22⟩ ⟧, #3, #4, #5, #6, #true, #false)
    → ⟦ {{⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
      ⟨Instructions CompileE(#1, #22, #3, #4, #5, RemoveHeadInRegList(#6))⟩}
      CMP ⟨Reg GetHeadInRegList(#6)⟩, ⟨Reg GetHeadInRegList(RemoveHeadInRegList(#6))⟩
  ...
  CompileCE(#1, ⟦ ⟨Expression#21⟩ && ⟨Expression#22⟩ ⟧, #3, #4, #5, #6, #true, #false)
    → ⟦ {⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
      ⟨Label#true⟩:
      ⟨Instructions CompileE(#1, #22, #3, #4, #5, #6)⟩
  ⟧;
  CompileCE(#1, ⟦ ⟨Expression#21⟩ || ⟨Expression#22⟩ ⟧, #3, #4, #5, #6, #true, #false)
    → ⟦ {⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
      ⟨Label#false⟩:
      ⟨Instructions CompileE(#1, #22, #3, #4, #5, #6)⟩
  ⟧;
  CompileCE(#1, #2, #3, #4, #5, #6, #true, #false) → CompileE(#1, #2, #3, #4, #5, #6);

  sort Computed | scheme MemberNameConCat2(Type, IDENTIFIER);
  MemberNameConCat2(#t, #2) → error ⟦No ConCat is allowed on non IDENTIFIER type⟧;
  default MemberNameConCat2( ⟦⟨IDENTIFIER#1⟩⟧, #2) → MemberNameConCat(#1, #2); // ⟦#1 @ "_" @ #2 @ "_offset"⟧;
  
  // helper scheme to compile an ExpressionList and output the arm code of each expression in the list
  sort Instructions | scheme CompileEinEL(PassOne, ExpressionList, Label, Computed, Emaps, RegList);
  CompileEinEL(#1, ⟦⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileEinEL(#1, ⟦ ⟨Expression#21⟩ ⟨ExpressionListTail#22⟩ ⟧, #3, #4, #5, #6)
    → ⟦ {⟨Instructions CompileE(#1, #21, #3, #4, #5, #6)⟩}
    ⟨Instructions CompileEinELT(#1, #22, #3, #4, #5, RemoveHeadInRegList(#6))⟩
  ⟧;

  sort Instructions | scheme CompileEinELT(PassOne, ExpressionListTail, Label, Computed, Emaps, RegList);
  CompileEinELT(#1, ⟦⟧, #3, #4, #5, #6) → ⟦⟧;
  CompileEinELT(#1, ⟦, ⟨Expression#21⟩ ⟨ExpressionListTail#22⟩ ⟧, #3, #4, #5, #6)
    → ⟦ {⟨Instructions CompileE(#1, #21, #3, #4, #5, RemoveHeadInRegList(#6))⟩}
    ⟨Instructions CompileEinELT(#1, #22, #3, #4, #5, RemoveHeadInRegList(RemoveHeadInRegList(#6)))⟩
  ⟧;
    
  // helpers to move Expression List into registers
  sort Instructions | scheme MovEL(ExpressionList, Reg, RegList);
  MovEL(⟦⟧, #reg, #6) → ⟦⟧;
  MovEL(⟦ ⟨Expression#21⟩ ⟨ExpressionListTail#22⟩ ⟧, #reg, #6) → ⟦
    MOV ⟨Reg #reg⟩, ⟨Reg GetHeadInRegList(#6)⟩
    ⟨Instructions MovELT(#22, GetNextReg(#reg), RemoveHeadInRegList(#6))⟩
  ⟧;

  sort Instructions | scheme MovELT(ExpressionListTail, Reg, RegList);
  MovELT(⟦⟧, #reg, #6) → ⟦⟧;
  MovELT(⟦, ⟨Expression#21⟩ ⟨ExpressionListTail#22⟩ ⟧, #reg, #6) → ⟦
    MOV ⟨Reg #reg⟩, ⟨Reg GetHeadInRegList(#6)⟩
    ⟨Instructions MovELT(#22, GetNextReg(#reg), RemoveHeadInRegList(#6))⟩
  ⟧;

  // Helper scheme to compile IDENTIFIER
  sort Instructions | scheme CompileId(IDENTIFIER, Emaps, RegList);
  CompileId(#2, #4, #6) → ⟦LDR ⟨Reg GetHeadInRegList(#6)⟩, [R12, #4]⟧;

  // sort Computed | scheme InEmaps();
  
  // Helper scheme to compile an Integer
  sort Instructions | scheme CompileInt(INTEGER, RegList);
  CompileInt(#2, #6) → CompileInt1(⟦$#2⟧,
		  ⟦ MOV ⟨Reg GetHeadInRegList(#6)⟩, #⟨INTEGER NegToPos(⟦$#2⟧)⟩ ⟧,
                  ⟦ LDR ⟨Reg GetHeadInRegList(#6)⟩, [PC, #0]
		    B N
		    DCI ⟨INTEGER#2⟩
		    N:
                  ⟧);
		  
  sort Instructions | scheme CompileInt1(Computed, Instructions, Instructions);
  CompileInt1(#2, #mov, #ldr) → CompileInt2(⟦ #2 <= 255 ? #mov : #ldr ⟧);

  sort Instructions | scheme CompileInt2(Computed);
  CompileInt2(#) → #;
    
  // helper schemes to resolve registers
  sort Reg | scheme GetHeadInRegList(RegList);
  GetHeadInRegList(⟦⟧) → error ⟦no more available unused registers⟧;
  GetHeadInRegList( ⟦ ⟨Reg#1⟩, ⟨RegList#2⟩⟧) → #1;

  sort RegList | scheme RemoveHeadInRegList(RegList);
  RemoveHeadInRegList(⟦⟧) → error ⟦Empty RegList already⟧;
  RemoveHeadInRegList(⟦ ⟨Reg#1⟩, ⟨RegList#2⟩⟧) → #2;

  // get the previous used registers
  sort Reg | scheme GetPrevReg(Reg);
  GetPrevReg(⟦R12⟧) → ⟦R11⟧;
  GetPrevReg(⟦R11⟧) → ⟦R10⟧;
  GetPrevReg(⟦R10⟧) → ⟦R9⟧;
  GetPrevReg(⟦R9⟧) → ⟦R8⟧;
  ...
  // [3] get the next unused registers
  sort Reg | scheme GetNextReg(Reg);
  GetNextReg(⟦R0⟧) → ⟦R1⟧;
  GetNextReg(⟦R1⟧) → ⟦R2⟧;
  GetNextReg(⟦R2⟧) → ⟦R3⟧;
  GetNextReg(⟦R3⟧) → ⟦R4⟧;
  ...
  GetNextReg(⟦R10⟧) → ⟦R11⟧;
  GetNextReg(⟦R11⟧) → ⟦R12⟧;
  GetNextReg(⟦R12⟧) → error ⟦ No Available Unused Registers⟧;
  
  // Offset calculation helper schemes
  sort Computed | scheme SubFour(Computed);
  SubFour(#num) → ⟦#num-4⟧;

  sort Computed | scheme AddFour(Computed);
  AddFour(#num) → ⟦#num+4⟧;

  sort Computed | scheme NegToPos(Computed);
  NegToPos(#num) → ⟦#num >= 0 ? #num : 0-#num⟧;

  // schemes for computing stack offset for sort U, S and Ss
  sort Computed | scheme ComputeUnit(Unit, Computed);
  ComputeUnit(⟦ ⟨Declaration#Ds⟩ ⟧, #offset) → #offset;
  ComputeUnit(⟦ ⟨Statement#S⟩ ⟧, #offset) → ComputeS(#S, #offset);

  sort Computed | scheme ComputeSs(Statements, Computed);
  ComputeSs(⟦⟧, #offset) → #offset;
  ComputeSs(⟦ ⟨Statement#11⟩ ⟨Statements#12⟩ ⟧, #offset)
    → ComputeSs(#12, ComputeS(#11, #offset));
  
  sort Computed | scheme ComputeS(Statement, Computed);
  ComputeS(⟦ ; ⟧, #offset) → #offset;
  ComputeS(⟦ ⟨Expression#⟩ ; ⟧, #offset) → #offset;
  ComputeS(⟦ return ; ⟧, #offset) → #offset;
  ComputeS(⟦ return ⟨Expression#⟩ ; ⟧, #offset) → #offset;
  ComputeS(⟦ while ( ⟨Expression#1⟩ ) ⟨Statement#2⟩ ⟧, #offset) → #offset;
  ComputeS(⟦ { ⟨Statements#⟩ } ⟧, #offset) → ComputeSs(#, #offset);
  ComputeS(⟦ if ( ⟨Expression#1⟩ ) ⟨IfTail#2⟩ ⟧, #offset) → #offset;
  ComputeS(⟦ var ⟨NameType#⟩ ; ⟧, #offset) → ⟦#offset-4⟧;

  // Other helper schemes to pass info down to the subtree
  sort Emaps | scheme ExtendSHelper(Statement, Computed, Emaps);
  ExtendSHelper(⟦ var ⟨NameType#1⟩ ; ⟧, #2, #3) → ⟦ ⟨NameType#1⟩ - ⟨INTEGER SubFour(#2)⟩ - ⟨Emaps#3⟩ ⟧;
  ExtendSHelper(⟦ ⟨Expression#1⟩ ; ⟧, #2, #3) →  #3;
  ExtendSHelper(⟦ ; ⟧, #2, #3) →  #3;
  ExtendSHelper(⟦ if ( ⟨Expression#11⟩ ) ⟨IfTail#12⟩ ⟧, #2, #3) →  #3;
  ExtendSHelper(⟦ while ( ⟨Expression#11⟩ ) ⟨Statement#12⟩ ⟧, #2, #3) →  #3;
  ExtendSHelper(⟦ return ⟨Expression#1⟩ ; ⟧, #2, #3) →  #3;
  ExtendSHelper(⟦ return ; ⟧, #2, #3) →  #3;
  ExtendSHelper(⟦ {⟨Statement#1⟩ } ⟧, #2, #3) → #3;

  sort Emaps | scheme AppendEmaps(Emaps, Emaps);
  AppendEmaps(⟦⟧, #2) → #2;
  AppendEmaps(⟦ ⟨Emap#11⟩ ⟨Emaps#12⟩ ⟧, #2) → ⟦ ⟨Emap#11⟩ ⟨Emaps AppendEmaps(#12, #2)⟩⟧;

  // helper scheme to find the next unused register
  sort Reg | scheme UnusedReg(Reg);
  
  /*
  // Compute expression to Reg#2
  | scheme CompileExpression(Expression, Reg);
  CompileExpression(⟦⟨INTEGER#1⟩⟧, Reg#2) → ⟦
    MOV ⟨Reg#2⟩, #⟨INTEGER#1⟩
  ⟧;
  */
